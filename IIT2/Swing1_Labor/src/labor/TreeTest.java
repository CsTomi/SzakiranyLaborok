...
public class TreeTest {
    ...
    public TreeTest() {
    	initPathSelector(); // Inicializálja a comboboxot
    }
    ...
    private void initPathSelector() {
    	...
    	// Az összes gyökér könyvtárat berakja elemnek
    	for (java.io.File item : java.io.File.listRoots()) {
    		// Csak olyan gyökérkönyvtárat teszünk be ami olvasható
    		if(item.canRead())
     ...
     * A combo box eseményeinek kezelésére szolgál.
     * 
     *
     */
    private class SelectorListener implements ActionListener {
    	
    	// Az elõzõ helyes útvonal
    	String prevPath = selector.getItemAt(0);

		@Override
		public void actionPerformed(ActionEvent e) {
			
			if (e.getActionCommand().equals("ROOT_CHANGED")) { // Új root
				...
				if (fileTreeModel.setRoot(tempRoot)) {
					...
					/*
					 * Azért oldom meg így, mert ha ciklus közepén raknám be az új elemet,
					 * akkor redundancia lép fel.
					 * Az oka egyszerû. Végig megy az összes elemen és a végére rakja az új elemet.
					 * Új elem elõtt van n db elem amire igaz, hogy nem = a berakandó elemmel.
					 * n alkalommal berakja ==> Bugos UI
					 * Ez a feature növel a komplexitáson, de pozitívabb a felhasználói élmény
					 */
					boolean isIn = false;
					...
					if (!isIn) 
						selector.addItem(prevPath); // Elõzménynek beállítja
				}
			}
		}
