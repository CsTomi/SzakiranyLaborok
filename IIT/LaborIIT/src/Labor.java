...
implements Runnable, ActionListener, PropertyChangeListener {
    ...
    public void propertyChange(PropertyChangeEvent evt) {
    	if (evt.getPropertyName().equals("progress")) {
    		...
    		if(evt.getNewValue() == SwingWorker.StateValue.DONE && evt.getSource().equals(u1) &&!u1.isCancelled()) {
	    	...
	    	} else if (evt.getNewValue() == SwingWorker.StateValue.DONE && evt.getSource().equals(u2) &&!u2.isCancelled()) {
    ...
    public void run() {
		...
		GroupLayout.SequentialGroup vGroup = layout.createSequentialGroup();
			vGroup.addGroup(layout.createParallelGroup(Alignment.CENTER).addComponent(label1).addComponent(field1).addComponent(b1).addComponent(progress1));
			vGroup.addGroup(layout.createParallelGroup(Alignment.CENTER).addComponent(label2).addComponent(field2).addComponent(b2).addComponent(progress2));
